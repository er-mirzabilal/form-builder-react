...
class Select extends React.Component {
  ...
  saveField = () => {
    const { name, machineName, order, conditionalLogic, isRequired, placeholder, helpText, subType, visibility, uuid, type, isMultiple, optionReferenceId, enableOther,parentId } = this.state
    let newField = {
      name,
      machineName,
      uuid,
      ...
      order,
      parentId,
      config: { conditionalLogic,isConditionalEnabled:!!conditionalLogic.conditions.length, isRequired, placeholder, helpText, subType, visibility, isMultiple, optionReferenceId, enableOther }
    }
    ...
    if (this.state.activeTab === 'basic') {
      return (
        <div>
          ...
          />
          {/*<Checkbox checked={this.state.isRequired} handleChange={val => this.handleDataChange(val, 'isRequired')}*/}
          {/*          label="Required"/>*/}
          {/*<div className="ahfb-mb-4">*/}
          {/*  <TextInput value={this.state.name} handleChange={e => this.handleDataChange(e.target.value, 'name')}*/}
          {/*             label="Field Name" required={true} autoFocus error={this.state.errors.name}/>*/}
          {/*</div>*/}
          {/*<div className="ahfb-mb-4">*/}
          {/*  <TextInput value={this.state.helpText} handleChange={e => this.handleDataChange(e.target.value, 'helpText')}*/}
          {/*             label="Help Text"/>*/}
          {/*</div>*/}
          {/*<div className="ahfb-mb-4">*/}
          {/*  <TextInput value={this.state.placeholder}*/}
          {/*             handleChange={e => this.handleDataChange(e.target.value, 'placeholder')} label="Placeholder"/>*/}
          {/*</div>*/}
          <div className="ahfb-mb-4">
          ...
          <div className="ahfb-mb-0">
            <Checkbox checked={this.state.isMultiple} handleChange={val => this.handleDataChange(val, 'isMultiple')}
      ...
      return (
        <div>
          <div className="ahfb-mb-4">
            <TextInput value={this.state.machineName}
                       handleChange={e => this.handleDataChange(e.target.value, 'machineName')}
                       label="Machine Name/Field ID" required={true} autoFocus error={this.state.errors.machineName}/>
          </div>
          <div className="ahfb-mb-4">
  ...
  render() {
    return (
      <div>
        <SimpleTabs value={this.state.activeTab} handleChange={this.handleTabChange}
                    options={[{ label: 'Basic Settings', value: 'basic' }, {
